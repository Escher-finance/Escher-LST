...
Babylon Chain: [Babylon Liquid Staking Contract](https://github.com/Escher-finance/cw-liquid-staking/tree/babylon/contracts/liquidstaking/liquidstaking-solo)

Union Chain: Separate implementation for the Union chain (you can see in the rest of the page).

Key Differences:

On the Babylon chain, integration is local. This means interactions with the contract are limited to within the Babylon chain, and potentially through IBC connections.

Reward management on Babylon is directly embedded within the staking contract, while on Union, rewards are managed by a separate contract. This design decision is due to Babylon's limitation that prevents changing the withdrawal reward address.

Babylon Contract Details:

The Babylon contract includes optimization mechanisms to maintain validator balance. Every 360 blocks, the contract automatically executes staking, unstaking, delegation, collect reward, operations to ensure stable performance.
