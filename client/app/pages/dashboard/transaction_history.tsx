        ...
        <Card className="w-full flex mt-6">
            ...
            <CardBody className="gap-4">
                {records && records.map((r: any) => {
                    return <div key={r.id}>{r.type.toUpperCase()} {r.amount} {r.type == "bond" ? network?.stakeCurrency.coinMinimalDenom : network?.stakeCurrency.liquidStakingDenom} at height: {r.height} <a href={`${explorer_url}${r.hash}`} target="_blank"> [Explorer]</a></div>
                })
                }
            </CardBody>
        </Card >
