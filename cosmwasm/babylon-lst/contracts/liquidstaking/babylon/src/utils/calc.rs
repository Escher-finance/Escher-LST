...
pub fn get_last_epoch_block(block: u64, epoch_period: u32) -> u64 {
    let remainder: u64 = block % u64::from(epoch_period);
    block - remainder
}
...
pub fn get_next_epoch(block: u64, epoch_period: u32) -> u64 {
    let remainder: u64 = block % u64::from(epoch_period);
    block + (u64::from(epoch_period) - remainder)
}
...
/// process the withdraw reward queue and return the new reward balance and new queue
#[must_use]
