...
import { tokenOrderV2, getSendbackCallMsg, getBabylonSendbackCallMsg, callInstruction, getInstructionBatch, encodeTokenOrderV2, encodeCall, tokenOrderV2Unescrow } from "./protocolV2.js";
...
const UNION_UCS03 = "union1336jj8ertl8h7rdvnz4dh5rqahd09cy0x43guhsxx6xyrztx292qpe64fh";
const UNION_LST_CONTRACT = "union1d2r4ecsuap4pujrlf3nz09vz8eha8y0z25knq0lfxz4yzn83v6kq0jxsmk";
const EU_ON_UNION_CONTRACT_ADDRESS = "union1eueueueu9var4yhdruyzkjcsh74xzeug6ckyy60hs0vcqnzql2hq0lxc2f";
const EU_ERC20 = "0xe5cf13c84c0fea3236c101bd7d743d30366e5cf1";
const UNION_ZKGM_MINTER_ADDRESS = "union1t5awl707x54k6yyx7qfkuqp890dss2pqgwxh07cu44x5lrlvt4rs8hqmk0";
...
let EBABY_ON_BABYLON_QUOTE_TOKEN: Address = "0x62626e31636e7833347038327a6e677130757561656e64736e65307834733567736d376770776b326573387a6b38727a38746e6a39333871717971386639"; // Denom ebaby in hex


const BABYLON_LST_CONTRACT = "bbn1ug4tume0pw6d4u7r6rhae6cp3udyrv7cr0angx8qegw7ur25sdxq4krcss";

//const BABYLON_LST_CONTRACT = "bbn1l2um8k69h9sd2amtypp0c44d5zxgylp8z84lmfujjvejw5k6frfswwr3wp"; //counter for testing only
...
export const createBondPayload = (
    { lst_address, mint_to_address, amount, min_mint_amount, denom }:
        {
            lst_address: string, mint_to_address: string, amount: string, min_mint_amount: string, denom: string
        }
) => {
    const bondMsg = {
        bond: {
...
export const createBabylonUnbondPayload = (
    { lst_address, recipient_address, amount, channel_id }:
        {
...
export const getUnionCallsInstruction = async (sender: string, amount: string, min_mint_amount: bigint, proxy_address: string, channel_id: number) => {
    ...
    let allowancePayload = createIncreaseAllowance(UNION_ZKGM_MINTER_ADDRESS, min_mint_amount.toString(), EU_ON_UNION_CONTRACT_ADDRESS);

    console.log(JSON.stringify(allowancePayload));

    const sendBackCallEffectPayload = await Effect.runPromise(getSendbackCallMsg({
...
export const getBabylonCallsInstruction = async (sender: string, amount: string, min_mint_amount: bigint, proxy_address: string, channel_id: number) => {
    ...
    let allowancePayload = createIncreaseAllowance(BABYLON_ZKGM_MINTER_ADDRESS, min_mint_amount.toString(), EBABY_ON_BABYLON_CONTRACT_ADDRESS);

    console.log(JSON.stringify(allowancePayload));

    const sendBackCallEffectPayload = await Effect.runPromise(getBabylonSendbackCallMsg({
        baseToken: EBABY_ON_BABYLON_CONTRACT_ADDRESS,
        channel_id: channel_id as number & Brand<"ChannelId">,
        metadata: toHex(""),
        minAmount: min_mint_amount,
...
export const getBabylonUnbondCallsInstruction = async (sender: string, amount: string, channel_id: number, proxy_address: string) => {

    // give allowance to lst contract to transfer ebaby from proxy contract
    let allowancePayload = createIncreaseAllowance(BABYLON_LST_CONTRACT, amount, EBABY_ON_BABYLON_CONTRACT_ADDRESS);
    ...
    // Calls
    const callsPayload = [
        allowancePayload, unbondPayload,
    ];
...
export const bondFromHoleskyToUnion = async (signer: ethers.Wallet, amount: bigint, proxy_address: string) => {
    ...
    let tokenOrder =
        tokenOrderV2(sender.toLowerCase(), proxy_address, U_ERC20, amount, AU_ON_UNION_QUOTE_TOKEN, amount);

    let calls = await getUnionCallsInstruction(sender, amount.toString(), min_mint_amount, proxy_address, UNION_TO_HOLESKY_CHANNEL_ID);
...
export const bondFromSepoliaToBabylon = async (signer: ethers.Wallet, amount: bigint, channel_id: number, proxy_address: string) => {
    let salt = getSalt();
    console.log(salt);

    const erc20Contract = new ethers.Contract(SEPOLIA_BBN_ERC20, erc20Abi, signer);
    const resp = await erc20Contract.approve(ETH_UCS03, amount);
    console.log(resp);
    ...
    let sender = await signer.getAddress();
    let rate = await fetchBabylonExchangeRate("exchange_rate");
    let min_mint_amount = BigInt(Math.floor(Number(amount) / Number(rate)));

    console.log("min_mint_amount:", min_mint_amount);

    let tokenOrder =
        tokenOrderV2Unescrow(sender.toLowerCase(), proxy_address, SEPOLIA_BBN_ERC20, amount, BBN_ON_BABYLON_QUOTE_TOKEN, amount);

    let calls = await getBabylonCallsInstruction(sender, amount.toString(), min_mint_amount, proxy_address, BABYLON_TO_SEPOLIA_CHANNEL_ID);
...
export const bondFromHoleskyToBabylon = async (signer: ethers.Wallet, amount: bigint, channel_id: number, proxy_address: string) => {
    ...
    //approve ucs03 contract to transfer first
    const erc20Contract = new ethers.Contract(HOLESKY_BBN_ERC20, erc20Abi, signer);
    const resp = await erc20Contract.approve(ETH_UCS03, amount);
    console.log(resp);
    ...
    let tokenOrder =
        tokenOrderV2Unescrow(sender.toLowerCase(), proxy_address, HOLESKY_BBN_ERC20, amount, BBN_ON_BABYLON_QUOTE_TOKEN, amount);

    let calls = await getBabylonCallsInstruction(sender, amount.toString(), min_mint_amount, proxy_address, BABYLON_TO_HOLESKY_CHANNEL_ID);
...
export const fetchBabylonExchangeRate = async (rate: string) => {
    try {
        const client = await CosmWasmClient.connect(BABYLON_RPC_URL)
        const response = await client.queryContractSmart(BABYLON_LST_CONTRACT, {
            staking_liquidity: {}
        })
        return response[rate];
    } catch (e) {
